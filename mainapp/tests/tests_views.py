...
# TODO:
# 4. (*) Покрыть тестами доступности все страницы сайта, в т.ч. переход по разным страницам со списком
...
class TestMainPage(TestCase):
    ...
    def test_page_open_with_auth(self):
        path = reverse("mainapp:index")
...
class TestCoursesPage(TestCase):
    ...
    def test_details_page_open(self):
        path = reverse("mainapp:courses_detail", kwargs={"pk": 1})
        result = self.client.get(path)
        self.assertEqual(result.status_code, HTTPStatus.OK)

    def test_details_page_open_with_auth(self):
        path = reverse("mainapp:courses_detail", kwargs={"pk": 1})
        result = self.client_with_auth.get(path)
        self.assertEqual(result.status_code, HTTPStatus.OK)
...
class TestCoursesFeedbackWithMock(TestCase):
    fixtures = (
        "authapp/fixtures/001_user_admin.json",
